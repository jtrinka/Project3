...
public class Assignment4 {
	public static void main(String[] args) {
	    ...
	    averagecalculationdialogbox(); //calls the method and passes newsize
	    nameentry();
		...
		//www.mathstackexchange.com
		   private static void averagecalculationdialogbox(int newsize) {
			   ...
			   int array[] = new int [newsize]; //creates a new array based off the size specified. This is parameter passing.
			    int sum=0;
			    int average=0;
			    	    for (int a=0; a<array.length;a++) //for an integer a which is equal to 0 to start, run the loop until a is greater than the array length. Add one to a after each time we go through the loop
			    	    {
			    	        array[a] = Integer.parseInt(JOptionPane.showInputDialog("Enter the grade each student got on their exam["+a+"].")); //prompt and entering a number into the array a
			    ...
			    average = sum/newsize; //calculates the average grade the class received.
			    	    ...
			    	    JOptionPane.showMessageDialog(null, "The average grade received was " + average, 
					            "The class average", JOptionPane.PLAIN_MESSAGE); //display result


			    }
		   private static void nameentry(int newsize){
		   String array2[] = new String[newsize];  //create a new array called array2 with the size of newsize array
